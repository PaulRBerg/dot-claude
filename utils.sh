...
# Add a Bash tool permission to Claude Code settings
function claude_allow_bash() {
  local tool_name="$1"

  if [[ -z "$tool_name" ]]; then
    echo "❌ Error: Tool name is required"
    echo "Usage: claude_allow_bash <tool-name>"
    return 1
  fi

  _claude_add_permission "Bash(${tool_name}:*)"
}

# Add a directory read permission to Claude Code settings
function claude_allow_read() {
  local dir_path="$1"

  if [[ -z "$dir_path" ]]; then
    echo "❌ Error: Directory path is required"
    echo "Usage: claude_allow_read <directory-path>"
    return 1
  fi

  # Normalize path: convert $HOME to ~ for consistency with existing permissions
  local normalized_path="${dir_path/#$HOME/\~}"
  _claude_add_permission "Read(${normalized_path})"
}

# Helper function to add a permission to Claude Code settings
function _claude_add_permission() {
  local permission="$1"
  ...
  if [[ -f "$settings_file" ]]; then
    ...
    if jq --arg perm "$permission" \
      '.permissions.allow += [$perm] | .permissions.allow |= sort' \
      "$settings_file" >"$temp_file"; then
      mv "$temp_file" "$settings_file"
      updated=true
    else
      rm -f "$temp_file"
      echo "⚠️  Warning: Failed to update $settings_file"
    fi
  fi
  ...
  if [[ "$updated" == true ]]; then
    ...
    echo "❌ Error: Settings file not found or update failed"
    return 1
  fi
}
