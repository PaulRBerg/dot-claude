#!/usr/bin/env python3
"""Unit tests for sync_settings.py hook."""
...
class TestStripJsoncComments:
  ...
  "key": "value"
        ...
        result = sync_settings.strip_jsonc_comments(content)
    ...
    def test_handles_empty_content(self):
        ...
        result = sync_settings.strip_jsonc_comments("")
        assert result == ""
...
class TestReadJsonc:
  ...
  "key": "value"
        ...
        result = sync_settings.read_jsonc(Path("/fake/path.jsonc"))
    ...
    def test_raises_on_invalid_json(self, mock_read):
        ...
        with pytest.raises(json.JSONDecodeError):
            sync_settings.read_jsonc(Path("/fake/path.jsonc"))


class TestDiscoverLocalSkills:
    """Test discover_local_skills() function."""
    ...
    def test_finds_skills_with_skill_md(self, mock_exists, mock_glob):
        ...
        result = sync_settings.discover_local_skills(Path("/fake/skills"))
    ...
    def test_returns_empty_for_missing_directory(self, mock_exists):
        ...
        result = sync_settings.discover_local_skills(Path("/nonexistent"))
        assert result == []
...
class TestExtractPluginSkills:
    ...
    def test_extracts_skills_with_colon(self):
        ...
        result = sync_settings.extract_plugin_skills(config)
    ...
    def test_handles_empty_config(self):
        ...
        result = sync_settings.extract_plugin_skills({})
        assert result == []

    def test_handles_missing_permissions(self):
        """Test handling config without permissions key."""
        result = sync_settings.extract_plugin_skills({"other": "key"})
        assert result == []
...
class TestMergeAndDedupeSkills:
    ...
    def test_merges_and_sorts(self):
        ...
        result = sync_settings.merge_and_dedupe_skills(local, plugin)
    ...
    def test_handles_empty_lists(self):
        ...
        result = sync_settings.merge_and_dedupe_skills([], [])
        assert result == []
...
class TestBuildSkillsJsonc:
    ...
    def test_includes_schema_and_comment(self):
        ...
        skills = ["Skill(test)"]
        result = sync_settings.build_skills_jsonc(skills)
...
class TestSyncSkills:
    ...
    @patch.object(sync_settings, "SKILLS_SETTINGS")
    def test_returns_false_when_settings_missing(self, mock_path):
        """Test returning False when skills.jsonc doesn't exist."""
        mock_path.exists.return_value = False
        result = sync_settings.sync_skills()
        assert result is False

    @patch.object(sync_settings, "SKILLS_SETTINGS")
    @patch.object(sync_settings, "CLAUDE_DIR", Path("/fake"))
    @patch("sync_settings.discover_local_skills")
    @patch("sync_settings.read_jsonc")
...
class TestDiscoverLocalCommands:
    """Test discover_local_commands() function."""
    ...
    def test_finds_commands_with_md_files(self, mock_exists, mock_glob):
        ...
        result = sync_settings.discover_local_commands(Path("/fake/commands"))
    ...
    def test_returns_empty_for_missing_directory(self, mock_exists):
        """Test returning empty list when directory doesn't exist."""
        mock_exists.return_value = False
        result = sync_settings.discover_local_commands(Path("/nonexistent"))
        assert result == []
...
class TestExtractPluginCommands:
    ...
    def test_extracts_commands_with_colon_in_path(self):
        ...
        result = sync_settings.extract_plugin_commands(config)
    ...
    def test_handles_empty_config(self):
        """Test handling empty configuration."""
        result = sync_settings.extract_plugin_commands({})
        assert result == []

    def test_handles_missing_permissions(self):
        """Test handling config without permissions key."""
        result = sync_settings.extract_plugin_commands({"other": "key"})
        assert result == []
...
class TestMergeAndDedupeCommands:
    ...
    def test_merges_and_sorts(self):
        ...
        result = sync_settings.merge_and_dedupe_commands(local, plugin)
        assert result == [
            "SlashCommand(/a-cmd:*)",
            "SlashCommand(/m-cmd:*)",
            "SlashCommand(/z-cmd:*)",
    ...
    def test_handles_empty_lists(self):
        """Test handling empty input lists."""
        result = sync_settings.merge_and_dedupe_commands([], [])
        assert result == []
...
class TestBuildCommandsJsonc:
    ...
    def test_includes_schema_and_comment(self):
        ...
        result = sync_settings.build_commands_jsonc(commands)
...
class TestSyncCommands:
    ...
    @patch.object(sync_settings, "COMMANDS_SETTINGS")
    def test_returns_false_when_settings_missing(self, mock_path):
        """Test returning False when commands.jsonc doesn't exist."""
        mock_path.exists.return_value = False
        result = sync_settings.sync_commands()
        assert result is False

    @patch.object(sync_settings, "COMMANDS_SETTINGS")
    @patch.object(sync_settings, "CLAUDE_DIR", Path("/fake"))
    @patch("sync_settings.discover_local_commands")
    @patch("sync_settings.read_jsonc")
    @patch("pathlib.Path.cwd")
...
class TestMergePlugins:
    ...
    def test_local_wins_on_conflict(self):
        ...
        result = sync_settings.merge_plugins(root, local)
...
class TestSyncPlugins:
    ...
    @patch.object(sync_settings, "ROOT_SETTINGS")
    @patch.object(sync_settings, "PLUGINS_SETTINGS")
    def test_returns_false_when_files_missing(self, mock_plugins, mock_root):
        ...
        result = sync_settings.sync_plugins()
        assert result is False

    @patch.object(sync_settings, "PLUGINS_SETTINGS")
    @patch.object(sync_settings, "ROOT_SETTINGS")
    @patch("sync_settings.read_plugins_from_root")
    @patch("sync_settings.read_plugins_from_local")
...
class TestMain:
    ...
    @patch("sync_settings.run_merge_settings")
    @patch("sync_settings.sync_plugins")
    @patch("sync_settings.sync_commands")
    @patch("sync_settings.sync_skills")
